...
use ark_ec::{pairing::{Pairing, PairingOutput}, Group};
use ark_serialize::*;
use ark_std::UniformRand;
use crate::{setup::AggregateKey, kzg::UniversalParams};
...
impl<E: Pairing> Ciphertext<E> {
    pub fn new(gamma_g2: E::G2, sa1: [E::G1; 2],  sa2: [E::G2; 6], enc_key: PairingOutput<E>) -> Self {
        Ciphertext { gamma_g2, sa1, sa2, enc_key }
    }
}

/// t is the threshold for encryption and apk is the aggregated public key
pub fn encrypt<E: Pairing>(msg: &[u8; 32], apk: &AggregateKey<E>, t:usize, params: &UniversalParams<E>) -> Ciphertext<E> {
    ...
    s.iter_mut().for_each(|s| *s = E::ScalarField::rand(&mut rng));

    // sa1[0] = s0*ask + s3*g^{tau^t} + s4*g^{-1}
    ...
    // sa2[5] = s4*g^{tau - omega^0}
    sa2[5] = (params.powers_of_h[1] + (params.powers_of_h[0] *(-E::ScalarField::one()))) * s[4];

    // enc_key = s4*e_gh
    let enc_key = e_gh.mul(s[4]);

    Ciphertext::new(gamma_g2, sa1, sa2, enc_key)
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::{kzg::KZG10, setup::{SecretKey, PublicKey}};
    ...
    fn test_encryption() {
        let mut rng = ark_std::test_rng();
        let n = 8;
        let params = KZG10::<E, UniPoly381>::setup(n+3, &mut rng).unwrap();
        ...
        let _ct = encrypt::<E>(&[1u8; 32], &ak, 2, &params);
    }
}